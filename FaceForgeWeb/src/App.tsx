...
const AdminScreen = ({ navigate }: any) => {
  ...
  const doAWSSync=()=>{speak("Syncing to AWS DataLake 3.0.");setLocked(true);setTimeout(()=>{speak("Sync complete. Purging local databanks.");setTimeout(()=>{localStorage.clear();window.location.reload();},2000);},3000);};
  return(<div className={`fill animate-fade-in ${locked?'lockdown-active':''}`} style={{background:'#060c1a',padding:'24px 20px',display:'flex',flexDirection:'column'}}><div style={{display:'flex',alignItems:'center',marginBottom:24}}><button onClick={()=>!locked&&navigate()} style={{background:'none',border:'none',color:'#fff',fontSize:14,fontWeight:800,cursor:'pointer',opacity:locked?0.4:1}}>← Close</button><span style={{flex:1,textAlign:'center',color:'#e53e3e',fontFamily:'monospace',fontSize:16,fontWeight:900,marginRight:50}}>ROOT_TERMINAL</span></div><div style={{display:'flex',gap:10,marginBottom:20}}><button onClick={doExport} disabled={locked} style={{flex:1,background:'rgba(0,136,255,0.15)',color:'#4da6ff',border:'1px solid rgba(0,136,255,0.3)',padding:'16px',borderRadius:16,fontWeight:800,cursor:'pointer',fontSize:11}}>📥 Export</button><button onClick={doAWSSync} disabled={locked} style={{flex:1,background:'rgba(255,153,0,0.15)',color:'#ff9900',border:'1px solid rgba(255,153,0,0.3)',padding:'16px',borderRadius:16,fontWeight:800,cursor:'pointer',fontSize:11}}>☁️ AWS Sync</button><button onClick={doPurge} disabled={locked} style={{flex:1,background:'rgba(255,59,48,0.15)',color:'#ff6b6b',border:'1px solid rgba(255,59,48,0.3)',padding:'16px',borderRadius:16,fontWeight:800,cursor:'pointer',fontSize:11}}>🚨 PURGE</button></div><div style={{color:'#00ff88',fontFamily:'monospace',fontSize:12,fontWeight:800,marginBottom:12}}>&gt; STORAGE DUMP</div><div style={{flex:1,background:'rgba(0,255,136,0.05)',border:'1px solid rgba(0,255,136,0.2)',borderRadius:16,padding:'16px',overflowY:'auto'}}><pre style={{color:'#00ff88',fontFamily:'monospace',fontSize:11,whiteSpace:'pre-wrap',lineHeight:1.6}}>{rawData}</pre></div>{locked&&<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(6,12,26,0.95)',backdropFilter:'blur(20px)',zIndex:100,flexDirection:'column',gap:16,textAlign:'center',padding:20}}><div style={{fontSize:64}}>🚨</div><h1 style={{color:'#e53e3e',fontSize:32,fontWeight:900,letterSpacing:-1}}>SYSTEM OVERRIDE</h1><p style={{color:'rgba(255,255,255,0.7)',fontSize:16,fontWeight:600}}>Processing secure operation...</p></div>}</div>);
};
